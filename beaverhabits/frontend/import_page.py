...
async def merge_habits(user: User, from_habit_list: HabitList):
    current_habit_list = await user_storage.get_user_habit_list(user)
    if current_habit_list is None:
        await user_storage.save_user_habit_list(user, from_habit_list)
        logging.info(f"Imported {len(from_habit_list.habits)} new habits")
    else:
        ...
        merged = len(set(current_habit_list.habits) & set(from_habit_list.habits))
...
def import_ui_page(user: User):
    async def handle_upload(e: events.UploadEventArguments):
        try:
            ...
            current_habit_list = await user_storage.get_user_habit_list(user)
            if current_habit_list is None:
                message = f"Are you sure? All your current habits will be replaced with {len(from_habit_list.habits)} habits."
            else:
                added = len(from_habit_list.habits) - len(current_habit_list.habits)
                merged = len(set(current_habit_list.habits) & set(from_habit_list.habits))
                unchanged = len(current_habit_list.habits) - merged
            ...
            await merge_habits(user, from_habit_list)
            ui.notify(
