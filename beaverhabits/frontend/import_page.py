...
def import_from_json(text: str) -> DictHabitList:
    d = json.loads(text)
...
async def merge_habits(user: User, to_habit_list: DictHabitList):
    ...
    if current_habit_list is None:
        ...
        merged_habit_list = await current_habit_list.merge(to_habit_list)
