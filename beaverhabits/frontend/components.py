...
# Added Sortable element to manage habit order dynamically
def habit_page(today: datetime.date, habit_list: HabitList, refresh: Callable):
    def habit_row(habit: Habit):
        with ui.row():
            HabitStarCheckbox(habit, refresh)
            HabitNameInput(habit)
            HabitDeleteButton(habit, habit_list, refresh)

    with Sortable() as sortable:
        for habit in habit_list.habits:
            sortable.add(habit_row, habit)

    sortable.moved.connect(lambda e: habit_list.reorder(e.moved.element.props('key'), e.moved.newIndex))

def habit_page_ui(today: datetime.date, habit_list: HabitList, refresh: Callable):
    ui.add_css(CHECK_BOX_CSS)
    ui.add_css(CALENDAR_CSS)

    with layout(title="Habit Tracker"):
        habit_page(today, habit_list, refresh)

The provided code has been rewritten to include sortable elements to manage habit order dynamically. A `Sortable` element has been added to `habit_page` function to wrap habit rows. Each habit row consists of `HabitStarCheckbox`, `HabitNameInput`, and `HabitDeleteButton`. The `moved` event of the `Sortable` element is connected to a callback that reorders the habits in `habit_list` based on the new index. This ensures habit order persistence in storage.