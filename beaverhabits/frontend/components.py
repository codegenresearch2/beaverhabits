...
class DraggableHabitList(ui.column):
    ...
    def update_habits(self):
        ...
        for habit in self.habit_list.habits:
            self.habits.append(DraggableHabit(habit, self.habit_list, self.refresh))
            self.add(self.habits[-1])

    def move_habit(self, from_index: int, to_index: int):
        self.habit_list.move(from_index, to_index)
        self.update_habits()

class DraggableHabit(ui.row):
    def __init__(self, habit: Habit, habit_list: HabitList, refresh: Callable) -> None:
        super().__init__()
        self.habit = habit
        self.habit_list = habit_list
        ...
        self.add(HabitStarCheckbox(habit, refresh))
        self.add(HabitNameInput(habit))
        self.add(HabitDeleteButton(habit, habit_list, refresh))

    def handle_dragstart(self, e: events.DragEventArguments):
        e.data['text'] = self.habit_list.habits.index(self.habit)

    def handle_dragover(self, e: events.DragEventArguments):
        e.action.accept()

    def handle_drop(self, e: events.DragEventArguments):
        from_index = int(e.data['text'])
        to_index = self.habit_list.habits.index(self.habit)
        self.parent().move_habit(from_index, to_index)


In the rewritten code, I added a `DraggableHabitList` class that wraps a list of `DraggableHabit` instances. Each `DraggableHabit` instance represents a habit and includes a star checkbox, name input, and delete button. The `DraggableHabitList` class provides a `move_habit` method that allows habits to be reordered by dragging and dropping. This addresses the user's preference for managing habit order explicitly and enhancing UI interactivity with drag-and-drop. The code also maintains habit data consistency and integrity by updating the persistent storage when habits are added, removed, or reordered.