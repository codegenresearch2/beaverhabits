...
from nicegui.elements.drag import Draggable, DropTarget
...
# Enhancing UI interactivity with drag-and-drop for habit ordering
class HabitListCard(ui.card):
    def __init__(self, habit_list: HabitList) -> None:
        super().__init__()
        self.habit_list = habit_list
        self.refresh()
        ...
        with self.tight():
            with ui.column():
                for habit in self.habit_list.habits:
                    with ui.row():
                        Draggable(habit.name)
                        HabitStarCheckbox(habit, self.refresh)
                        HabitDeleteButton(habit, self.habit_list, self.refresh)
            HabitAddButton(self.habit_list, self.refresh)

    def on_drag_end(self, e: events.DragEndEventArguments):
        source_index = e.source.index
        target_index = e.target.index
        self.habit_list.move(source_index, target_index)

    def on_drop(self, e: events.DropEventArguments):
        source_index = e.source.index
        target_index = e.target.index
        self.habit_list.move(source_index, target_index)

# Maintaining habit data consistency and integrity
async def tick(self, day: datetime.date, value: bool):
    if day in self.ticked_days and not value:
        self.ticked_days.remove(day)
    elif day not in self.ticked_days and value:
